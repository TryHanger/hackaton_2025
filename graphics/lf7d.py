...
def compare_with_join():
    ...
    try:
        ...
        result_cursor.execute("""
            CREATE TABLE login_frequency_7d AS
            SELECT 
                i.cst_dim_id,
                i.transdate,
                i.login_frequency_7d,
                p.target
            ...
            WHERE i.login_frequency_7d IS NOT NULL
        """)

        # Статистика
        result_cursor.execute("SELECT COUNT(*) FROM login_frequency_7d")
        total_count = result_cursor.fetchone()[0]

        result_cursor.execute("SELECT COUNT(*) FROM login_frequency_7d WHERE target IS NOT NULL")
        matched_count = result_cursor.fetchone()[0]

        result_cursor.execute("SELECT COUNT(*) FROM login_frequency_7d WHERE target = 1")
        target_1_count = result_cursor.fetchone()[0]

        result_cursor.execute("SELECT COUNT(*) FROM login_frequency_7d WHERE target = 0")
        ...
        # Выборка результатов
        result_cursor.execute("SELECT * FROM login_frequency_7d LIMIT 10")
        sample_results = result_cursor.fetchall()

        print("\n🔍 ПЕРВЫЕ 10 РЕЗУЛЬТАТОВ:")
        print("ID | Дата | login_frequency_7d | Target")
        print("-" * 50)
        ...
        """)
        result_cursor.execute("""
            CREATE TABLE results_db.login_frequency_7d AS
            SELECT * FROM login_frequency_7d
        """)
...
def load_data():
    ...
    df = pd.read_sql_query(
        "SELECT login_frequency_7d, target FROM login_frequency_7d WHERE target IS NOT NULL AND login_frequency_7d IS NOT NULL",
        conn
    )
    conn.close()

    print(f"📊 ДАННЫЕ ДЛЯ АНАЛИЗА (login_frequency_7d):")
    print(f"Диапазон значений: {df['login_frequency_7d'].min():.2f} - {df['login_frequency_7d'].max():.2f}")
...
def automatic_binning_analysis(df):
    ...
    X = df['login_frequency_7d'].values.reshape(-1, 1)
    ...
    for idx, (strategy, name) in enumerate(strategies.items()):
        ...
        bin_stats = df_binned.groupby('bin').agg({
            'login_frequency_7d': ['min', 'max', 'count'],
            'target': ['sum', 'mean']
        }).round(3)
...
def trend_analysis(df):
    """Статистический анализ тренда риска для login_frequency_7d."""
    ...
    df['bin'] = pd.cut(df['login_frequency_7d'], bins=bins, labels=labels, right=False)
    trend_data = df.groupby('bin').agg({
        'target': ['count', 'sum', 'mean'],
        'login_frequency_7d': 'median'
    }).round(4)
    ...
    ax1.set_title('Анализ тренда Fraud Rate\n(login_frequency_7d)', fontsize=16, fontweight='bold')
...
def tail_analysis(df):
    ...
    mean_logins = df['login_frequency_7d'].mean()
    std_logins = df['login_frequency_7d'].std()
    outlier_threshold = mean_logins + 3 * std_logins

    outliers = df[df['login_frequency_7d'] > outlier_threshold]
    normal_data = df[df['login_frequency_7d'] <= outlier_threshold]
    ...
    for threshold in extreme_thresholds:
        extreme_data = df[df['login_frequency_7d'] >= threshold]
    ...
    ax1.hist(normal_data['login_frequency_7d'], bins=50, alpha=0.7,
             color='blue', label=f'Нормальные (≤{outlier_threshold:.0f})')
    ax1.hist(outliers['login_frequency_7d'], bins=20, alpha=0.7,
    ...
    percentile_values = np.percentile(df['login_frequency_7d'], percentiles)
    percentile_fraud = []

    for p in percentiles:
        threshold = np.percentile(df['login_frequency_7d'], p)
        high_activity = df[df['login_frequency_7d'] >= threshold]
    ...
    for p in key_percentiles:
        ...
        actual_threshold = np.percentile(df['login_frequency_7d'], p)
    ...
    for p in [99, 99.5, 99.9]:
        threshold = np.percentile(df['login_frequency_7d'], p)
        top_users = df[df['login_frequency_7d'] >= threshold]
        if len(top_users) > 0:
...
def nonlinear_analysis(df):
    """Поиск нелинейных зависимостей для login_frequency_7d."""
    ...
    sorted_df = df.copy().sort_values('login_frequency_7d').reset_index(drop=True)
    ...
    X_poly = valid_data['login_frequency_7d'].values
    ...
    ax1.scatter(sorted_df['login_frequency_7d'], sorted_df['target'],
                alpha=0.1, color='gray', s=1, label='Исходные данные')
    ax1.plot(valid_data['login_frequency_7d'], valid_data['rolling_fraud'] * 100,
...
def kde_analysis(df):
    """Kernel Density Estimation для распределений login_frequency_7d."""

    print("\n" + "=" * 60)
    print("5. KERNEL DENSITY ESTIMATION")
    print("=" * 60)

    freq_0 = df[df['target'] == 0]['login_frequency_7d']
    freq_1 = df[df['target'] == 1]['login_frequency_7d']

    # Адаптивное ограничение для визуализации
    max_val = min(15, df['login_frequency_7d'].max() * 1.1)  # Немного больше максимума
...
def final_report(df, trend_data, r_squared, likelihood_ratio, x_range):
    """Финальный отчет с выводами для login_frequency_7d."""

    print("\n" + "=" * 80)
    print("🎯 ИТОГОВЫЙ ОТЧЕТ АНАЛИЗА (login_frequency_7d)")
    print("=" * 80)

    # Ключевые метрики
    total_fraud_rate = df['target'].mean() * 100
    correlation = df['login_frequency_7d'].corr(df['target'])
    ...
    high_risk_data = df[df['login_frequency_7d'] >= high_risk_threshold]
    medium_risk_data = df[(df['login_frequency_7d'] >= medium_risk_threshold) &
                          (df['login_frequency_7d'] < high_risk_threshold)]
    ...
    # KDE распределения
    max_kde = min(15, df['login_frequency_7d'].max() * 1.1)
...
if __name__ == '__main__':
    if num == 0:
        ...
        df = load_data()
        logins_0 = df[df['target'] == 0]['login_frequency_7d']
        logins_1 = df[df['target'] == 1]['login_frequency_7d']
        ...
        # stats_df = calculate_detailed_stats(df, value_column='login_frequency_7d', target_column='target')
        ...
        visualize_bin_analysis(result, 'login_frequency_7d')