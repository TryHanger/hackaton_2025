...
def compare_with_join():
    ...
    try:
        ...
        result_cursor.execute("""
            CREATE TABLE logins_7d_over_30d_ratio AS
            SELECT 
                i.cst_dim_id,
                i.transdate,
                i.logins_7d_over_30d_ratio,
                p.target
            ...
            WHERE i.logins_7d_over_30d_ratio IS NOT NULL
        """)

        # Статистика
        result_cursor.execute("SELECT COUNT(*) FROM logins_7d_over_30d_ratio")
        total_count = result_cursor.fetchone()[0]

        result_cursor.execute("SELECT COUNT(*) FROM logins_7d_over_30d_ratio WHERE target IS NOT NULL")
        matched_count = result_cursor.fetchone()[0]

        result_cursor.execute("SELECT COUNT(*) FROM logins_7d_over_30d_ratio WHERE target = 1")
        target_1_count = result_cursor.fetchone()[0]

        result_cursor.execute("SELECT COUNT(*) FROM logins_7d_over_30d_ratio WHERE target = 0")
        ...
        # Выборка результатов
        result_cursor.execute("SELECT * FROM logins_7d_over_30d_ratio LIMIT 10")
        sample_results = result_cursor.fetchall()

        print("\n🔍 ПЕРВЫЕ 10 РЕЗУЛЬТАТОВ:")
        print("ID | Дата | logins_7d_over_30d_ratio | Target")
        print("-" * 50)
        ...
        """)
        result_cursor.execute("""
            CREATE TABLE results_db.logins_7d_over_30d_ratio AS
            SELECT * FROM logins_7d_over_30d_ratio
        """)
...
def load_data():
    ...
    df = pd.read_sql_query(
        "SELECT logins_7d_over_30d_ratio, target FROM logins_7d_over_30d_ratio WHERE target IS NOT NULL AND logins_7d_over_30d_ratio IS NOT NULL",
        conn
    )
    conn.close()

    print(f"📊 ДАННЫЕ ДЛЯ АНАЛИЗА (logins_7d_over_30d_ratio):")
    print(f"Диапазон значений: {df['logins_7d_over_30d_ratio'].min():.2f} - {df['logins_7d_over_30d_ratio'].max():.2f}")
...
def automatic_binning_analysis(df):
    ...
    X = df['logins_7d_over_30d_ratio'].values.reshape(-1, 1)
    ...
    for idx, (strategy, name) in enumerate(strategies.items()):
        ...
        bin_stats = df_binned.groupby('bin').agg({
            'logins_7d_over_30d_ratio': ['min', 'max', 'count'],
            'target': ['sum', 'mean']
        }).round(3)
...
def trend_analysis(df):
    """Статистический анализ тренда риска для logins_7d_over_30d_ratio."""
    ...
    df['bin'] = pd.cut(df['logins_7d_over_30d_ratio'], bins=bins, labels=labels, right=False)
    trend_data = df.groupby('bin').agg({
        'target': ['count', 'sum', 'mean'],
        'logins_7d_over_30d_ratio': 'median'
    }).round(4)
    ...
    ax1.set_title('Анализ тренда Fraud Rate\n(logins_7d_over_30d_ratio)', fontsize=16, fontweight='bold')
...
def tail_analysis(df):
    ...
    mean_logins = df['logins_7d_over_30d_ratio'].mean()
    std_logins = df['logins_7d_over_30d_ratio'].std()
    outlier_threshold = mean_logins + 3 * std_logins

    outliers = df[df['logins_7d_over_30d_ratio'] > outlier_threshold]
    normal_data = df[df['logins_7d_over_30d_ratio'] <= outlier_threshold]
    ...
    for threshold in extreme_thresholds:
        extreme_data = df[df['logins_7d_over_30d_ratio'] >= threshold]
    ...
    ax1.hist(normal_data['logins_7d_over_30d_ratio'], bins=50, alpha=0.7,
             color='blue', label=f'Нормальные (≤{outlier_threshold:.0f})')
    ax1.hist(outliers['logins_7d_over_30d_ratio'], bins=20, alpha=0.7,
    ...
    percentile_values = np.percentile(df['logins_7d_over_30d_ratio'], percentiles)
    percentile_fraud = []

    for p in percentiles:
        threshold = np.percentile(df['logins_7d_over_30d_ratio'], p)
        high_activity = df[df['logins_7d_over_30d_ratio'] >= threshold]
    ...
    for p in key_percentiles:
        ...
        actual_threshold = np.percentile(df['logins_7d_over_30d_ratio'], p)
    ...
    for p in [99, 99.5, 99.9]:
        threshold = np.percentile(df['logins_7d_over_30d_ratio'], p)
        top_users = df[df['logins_7d_over_30d_ratio'] >= threshold]
        if len(top_users) > 0:
...
def nonlinear_analysis(df):
    """Поиск нелинейных зависимостей для logins_7d_over_30d_ratio."""
    ...
    sorted_df = df.copy().sort_values('logins_7d_over_30d_ratio').reset_index(drop=True)
    ...
    X_poly = valid_data['logins_7d_over_30d_ratio'].values
    ...
    ax1.scatter(sorted_df['logins_7d_over_30d_ratio'], sorted_df['target'],
                alpha=0.1, color='gray', s=1, label='Исходные данные')
    ax1.plot(valid_data['logins_7d_over_30d_ratio'], valid_data['rolling_fraud'] * 100,
...
def kde_analysis(df):
    """Kernel Density Estimation для распределений logins_7d_over_30d_ratio."""

    print("\n" + "=" * 60)
    print("5. KERNEL DENSITY ESTIMATION")
    print("=" * 60)

    freq_0 = df[df['target'] == 0]['logins_7d_over_30d_ratio']
    freq_1 = df[df['target'] == 1]['logins_7d_over_30d_ratio']

    # Адаптивное ограничение для визуализации
    max_val = min(15, df['logins_7d_over_30d_ratio'].max() * 1.1)  # Немного больше максимума
...
def final_report(df, trend_data, r_squared, likelihood_ratio, x_range):
    """Финальный отчет с выводами для logins_7d_over_30d_ratio."""

    print("\n" + "=" * 80)
    print("🎯 ИТОГОВЫЙ ОТЧЕТ АНАЛИЗА (logins_7d_over_30d_ratio)")
    print("=" * 80)

    # Ключевые метрики
    total_fraud_rate = df['target'].mean() * 100
    correlation = df['logins_7d_over_30d_ratio'].corr(df['target'])
    ...
    high_risk_data = df[df['logins_7d_over_30d_ratio'] >= high_risk_threshold]
    medium_risk_data = df[(df['logins_7d_over_30d_ratio'] >= medium_risk_threshold) &
                          (df['logins_7d_over_30d_ratio'] < high_risk_threshold)]
    ...
    # KDE распределения
    max_kde = min(15, df['logins_7d_over_30d_ratio'].max() * 1.1)
...
def analyze_manual_bins(df, feature, target, bins, bin_labels=None):
    ...
    df : pandas.DataFrame
        Исходный датафрейм
    feature : str
        Название числовой колонки (например 'logins_7d_over_30d_ratio')
    target : str
...
if __name__ == '__main__':
    if num == 0:
        ...
        df = load_data()
        logins_0 = df[df['target'] == 0]['logins_7d_over_30d_ratio']
        logins_1 = df[df['target'] == 1]['logins_7d_over_30d_ratio']
        ...
        # stats_df = calculate_detailed_stats(df, value_column='logins_7d_over_30d_ratio', target_column='target')
        # formatted_stats = print_detailed_stats(stats_df)

    elif num == 2:
        df = load_data()
        bins = [0.0, 0.37, 1.0]
        labels = ['Базовый риск', 'Повышенный риск']
        result = analyze_manual_bins(
            df=df,
            feature='logins_7d_over_30d_ratio',
            target='target',
            bins=bins,
            bin_labels=labels
        )

        visualize_bin_analysis(result, 'logins_7d_over_30d_ratio')