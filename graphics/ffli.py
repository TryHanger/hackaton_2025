...
def compare_with_join():
    ...
    try:
        ...
        result_cursor.execute("""
            CREATE TABLE fano_factor_login_interval AS
            SELECT 
                i.cst_dim_id,
                i.transdate,
                i.fano_factor_login_interval,
                p.target
            ...
            WHERE i.fano_factor_login_interval IS NOT NULL
        """)

        # Статистика
        result_cursor.execute("SELECT COUNT(*) FROM fano_factor_login_interval")
        total_count = result_cursor.fetchone()[0]

        result_cursor.execute("SELECT COUNT(*) FROM fano_factor_login_interval WHERE target IS NOT NULL")
        matched_count = result_cursor.fetchone()[0]

        result_cursor.execute("SELECT COUNT(*) FROM fano_factor_login_interval WHERE target = 1")
        target_1_count = result_cursor.fetchone()[0]

        result_cursor.execute("SELECT COUNT(*) FROM fano_factor_login_interval WHERE target = 0")
        ...
        # Выборка результатов
        result_cursor.execute("SELECT * FROM fano_factor_login_interval LIMIT 10")
        sample_results = result_cursor.fetchall()

        print("\n🔍 ПЕРВЫЕ 10 РЕЗУЛЬТАТОВ:")
        print("ID | Дата | fano_factor_login_interval | Target")
        print("-" * 50)
        ...
        """)
        result_cursor.execute("""
            CREATE TABLE results_db.fano_factor_login_interval AS
            SELECT * FROM fano_factor_login_interval
        """)
...
def load_data():
    ...
    df = pd.read_sql_query(
        "SELECT fano_factor_login_interval, target FROM fano_factor_login_interval WHERE target IS NOT NULL",
        conn
    )
    conn.close()
    return df
...
def automatic_binning_analysis(df):
    ...
    X = df['fano_factor_login_interval'].values.reshape(-1, 1)
    ...
    for idx, (strategy, name) in enumerate(strategies.items()):
        ...
        bin_stats = df_binned.groupby('bin').agg({
            'fano_factor_login_interval': ['min', 'max', 'count'],
            'target': ['sum', 'mean']
        }).round(3)
...
def trend_analysis(df):
    ...
    df['bin'] = pd.cut(df['fano_factor_login_interval'], bins=bins, labels=labels, right=False)
    trend_data = df.groupby('bin').agg({
        'target': ['count', 'sum', 'mean'],
        'fano_factor_login_interval': 'median'
    }).round(4)
...
def tail_analysis(df):
    ...
    mean_logins = df['fano_factor_login_interval'].mean()
    std_logins = df['fano_factor_login_interval'].std()
    outlier_threshold = mean_logins + 3 * std_logins

    outliers = df[df['fano_factor_login_interval'] > outlier_threshold]
    normal_data = df[df['fano_factor_login_interval'] <= outlier_threshold]
    ...
    for threshold in extreme_thresholds:
        extreme_data = df[df['fano_factor_login_interval'] >= threshold]
    ...
    ax1.hist(normal_data['fano_factor_login_interval'], bins=50, alpha=0.7,
             color='blue', label=f'Нормальные (≤{outlier_threshold:.0f})')
    ax1.hist(outliers['fano_factor_login_interval'], bins=20, alpha=0.7,
    ...
    percentile_values = np.percentile(df['fano_factor_login_interval'], percentiles)
    percentile_fraud = []

    for p in percentiles:
        threshold = np.percentile(df['fano_factor_login_interval'], p)
        high_activity = df[df['fano_factor_login_interval'] >= threshold]
    ...
    for p in key_percentiles:
        ...
        actual_threshold = np.percentile(df['fano_factor_login_interval'], p)
    ...
    for p in [99, 99.5, 99.9]:
        threshold = np.percentile(df['fano_factor_login_interval'], p)
        top_users = df[df['fano_factor_login_interval'] >= threshold]
        if len(top_users) > 0:
...
def nonlinear_analysis(df):
    ...
    sorted_df = df.copy().sort_values('fano_factor_login_interval').reset_index(drop=True)
    ...
    X_poly = valid_data['fano_factor_login_interval'].values
    ...
    ax1.scatter(sorted_df['fano_factor_login_interval'], sorted_df['target'],
                alpha=0.1, color='gray', s=1, label='Исходные данные')
    ax1.plot(valid_data['fano_factor_login_interval'], valid_data['rolling_fraud'] * 100,
...
def kde_analysis(df):
    ...
    logins_0 = df[df['target'] == 0]['fano_factor_login_interval']
    logins_1 = df[df['target'] == 1]['fano_factor_login_interval']
    ...
    # Безопасная обёртка для gaussian_kde: обрабатывает случаи с малым
    # количеством наблюдений или константными значениями.
    def make_safe_kde(values, default_max=max_val):
        vals = np.asarray(values.dropna()) if hasattr(values, 'dropna') else np.asarray(values)
        if vals.size == 0:
            # пустые данные — возвращаем нулевую функцию
            return lambda x: np.zeros_like(x)
        if vals.size == 1 or np.allclose(vals, vals.flat[0]):
            # один уникальный элемент — заменить на небольшую ширину вокруг точки
            center = float(vals.flat[0])
            def single_kde(x, c=center):
                # маленький гауссов профиль вокруг центра
                return np.exp(-0.5 * ((x - c) / 0.5) ** 2) / (0.5 * np.sqrt(2 * np.pi))
            return single_kde
        # Пытаемся создать gaussian_kde, при ошибке добавляем небольшую случайную джиттеризацию
        try:
            kde = stats.gaussian_kde(vals)
            return kde
        except Exception:
            try:
                vals_j = vals + np.random.normal(0, 1e-6, size=vals.shape)
                kde = stats.gaussian_kde(vals_j)
                return kde
            except Exception:
                # fallback: гистограммный оцениватель
                hist, bin_edges = np.histogram(vals, bins=50, range=(0, default_max), density=True)
                def hist_kde(x, hist=hist, edges=bin_edges):
                    idx = np.clip(np.digitize(x, edges) - 1, 0, len(hist)-1)
                    return hist[idx]
                return hist_kde

    # Построим KDE безопасно и отобразим несколько bandwidth-ов (если возможно)
    fig, axes = plt.subplots(2, 2, figsize=(15, 12))
    bandwidths = [0.5, 1.0, 2.0, 'auto']
    titles = ['Small bandwidth (0.5)', 'Medium bandwidth (1.0)',
              'Large bandwidth (2.0)', 'Auto/default bandwidth']

    # Подготовим массив x
    x_range = np.linspace(0, max_val, 200)

    # Создаём базовые KDE функции
    # Для ручных bw мы попытаемся использовать gaussian_kde с указанным bw_method,
    # но оборачиваем в try/except внутри make_safe_kde
    for idx, (bw, title) in enumerate(zip(bandwidths, titles)):
        ax = axes[idx // 2, idx % 2]

        # Попробуем применить bw метод, если это не 'auto'
        def build_kde_for(vals, bw):
            vals = np.asarray(vals.dropna()) if hasattr(vals, 'dropna') else np.asarray(vals)
            if vals.size < 2 or np.allclose(vals, vals.flat[0]):
                return make_safe_kde(vals)
            try:
                if bw == 'auto':
                    return stats.gaussian_kde(vals)
                else:
                    return stats.gaussian_kde(vals, bw_method=bw)
            except Exception:
                return make_safe_kde(vals)

        kde_0 = build_kde_for(logins_0_trimmed, bw)
        kde_1 = build_kde_for(logins_1_trimmed, bw)

        # Вычисляем плотности — KDE может быть либо объектом callable, либо функцией
        try:
            density_0 = kde_0(x_range)
        except Exception:
            density_0 = np.array([kde_0(xx) for xx in x_range])
        try:
            density_1 = kde_1(x_range)
        except Exception:
            density_1 = np.array([kde_1(xx) for xx in x_range])

        ax.plot(x_range, density_0, label='Target=0 (Легитимные)', color='blue', linewidth=2)
        ax.plot(x_range, density_1, label='Target=1 (Мошеннические)', color='red', linewidth=2)
    ...
    # Ratio of densities (используем безопасный авто-kde через make_safe_kde)
    kde_0_auto = make_safe_kde(logins_0_trimmed)
    kde_1_auto = make_safe_kde(logins_1_trimmed)

    try:
        density_0 = kde_0_auto(x_range)
    except Exception:
        density_0 = np.array([kde_0_auto(xx) for xx in x_range])
    try:
        density_1 = kde_1_auto(x_range)
    except Exception:
        density_1 = np.array([kde_1_auto(xx) for xx in x_range])

    # Отношение плотностей — защитимся от деления на ноль
    ...
    if len(fraud_preferred) > 0:
        ...
        peak_idx = np.nanargmax(likelihood_ratio)
...
def final_report(df, trend_data, r_squared, likelihood_ratio, x_range):
    ...
    correlation = df['fano_factor_login_interval'].corr(df['target'])
    ...
    high_risk_data = df[df['fano_factor_login_interval'] >= high_risk_threshold]
    medium_risk_data = df[(df['fano_factor_login_interval'] >= medium_risk_threshold) &
                          (df['fano_factor_login_interval'] < high_risk_threshold)]
    ...
    # Вычислим логины по таргету внутри функции, чтобы не зависеть от внешней области видимости
    logins_0 = df[df['target'] == 0]['fano_factor_login_interval']
    logins_1 = df[df['target'] == 1]['fano_factor_login_interval']
...
def calculate_detailed_stats(df, value_column='fano_factor_login_interval', target_column='target'):
    """
...
def print_detailed_stats(stats_df):
    ...
    print("\n" + "=" * 100)
    print("📊 ДЕТАЛИЗИРОВАННАЯ СТАТИСТИКА ДЛЯ fano_factor_login_interval")
    print("=" * 100)
...
if __name__ == '__main__':
    if num == 0:
        ...
        # stats_df = calculate_detailed_stats(df, value_column='fano_factor_login_interval', target_column='target')
        # formatted_stats = print_detailed_stats(stats_df)

    elif num == 2:
        df = load_data()
        # Группировка для fano_factor_login_interval
        bins = [-1.0, -0.7, 0.4, 0.683]
        labels = ['A. Высокий Риск', 'B. Средний Риск', 'C. Низкий Риск']
        result = analyze_manual_bins(
            df=df,
            feature='fano_factor_login_interval',
            target='target',
            bins=bins,
            bin_labels=labels
        )

        visualize_bin_analysis(result, 'fano_factor_login_interval')