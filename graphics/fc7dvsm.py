...
def compare_with_join():
    ...
    try:
        ...
        # SQL-выражение для обработки freq_change_7d_vs_mean
        login_freq_clean_sql = "i.freq_change_7d_vs_mean"
        ...
        result_cursor.execute(f"""
            CREATE TABLE freq_change_7d_vs_mean AS
            SELECT 
                i.cst_dim_id,
                i.transdate,
                {login_freq_clean_sql} AS freq_change_7d_vs_mean_clean,
                p.target
            ...
            WHERE freq_change_7d_vs_mean_clean IS NOT NULL
        """)

        # --- Теперь статистика и вывод используют новое чистое имя столбца ---

        # Статистика
        result_cursor.execute("SELECT COUNT(*) FROM freq_change_7d_vs_mean")
        total_count = result_cursor.fetchone()[0]

        result_cursor.execute("SELECT COUNT(*) FROM freq_change_7d_vs_mean WHERE target IS NOT NULL")
        matched_count = result_cursor.fetchone()[0]

        # NOTE: Теперь фильтруем по новому чистому столбцу!
        result_cursor.execute("SELECT COUNT(*) FROM freq_change_7d_vs_mean WHERE target = 1")
        target_1_count = result_cursor.fetchone()[0]

        result_cursor.execute("SELECT COUNT(*) FROM freq_change_7d_vs_mean WHERE target = 0")
        ...
        result_cursor.execute(
            "SELECT cst_dim_id, transdate, freq_change_7d_vs_mean_clean, target FROM freq_change_7d_vs_mean LIMIT 10")
        sample_results = result_cursor.fetchall()

        print("\n🔍 ПЕРВЫЕ 10 РЕЗУЛЬТАТОВ (Числовой столбец очищен):")
        print("ID | Дата | freq_change_7d_vs_mean_clean | Target")
        print("-" * 50)
        ...
        """)
        result_cursor.execute("""
            CREATE TABLE results_db.freq_change_7d_vs_mean AS
            SELECT cst_dim_id, transdate, freq_change_7d_vs_mean_clean AS freq_change_7d_vs_mean, target 
            FROM freq_change_7d_vs_mean
        """)
...
def load_data():
    ...
    df = pd.read_sql_query(
        "SELECT freq_change_7d_vs_mean, target FROM freq_change_7d_vs_mean WHERE target IS NOT NULL",
        conn
    )
    conn.close()
    return df
...
def automatic_binning_analysis(df):
    ...
    X = df['freq_change_7d_vs_mean'].values.reshape(-1, 1)
    ...
    for idx, (strategy, name) in enumerate(strategies.items()):
        ...
        bin_stats = df_binned.groupby('bin').agg({
            'freq_change_7d_vs_mean': ['min', 'max', 'count'],
            'target': ['sum', 'mean']
        }).round(3)
...
def trend_analysis(df):
    ...
    df['bin'] = pd.cut(df['freq_change_7d_vs_mean'], bins=bins, labels=labels, right=False)
    trend_data = df.groupby('bin').agg({
        'target': ['count', 'sum', 'mean'],
        'freq_change_7d_vs_mean': 'median'
    }).round(4)
...
def tail_analysis(df):
    ...
    mean_logins = df['freq_change_7d_vs_mean'].mean()
    std_logins = df['freq_change_7d_vs_mean'].std()
    outlier_threshold = mean_logins + 3 * std_logins

    outliers = df[df['freq_change_7d_vs_mean'] > outlier_threshold]
    normal_data = df[df['freq_change_7d_vs_mean'] <= outlier_threshold]
    ...
    for threshold in extreme_thresholds:
        extreme_data = df[df['freq_change_7d_vs_mean'] >= threshold]
    ...
    ax1.hist(normal_data['freq_change_7d_vs_mean'], bins=50, alpha=0.7,
             color='blue', label=f'Нормальные (≤{outlier_threshold:.0f})')
    ax1.hist(outliers['freq_change_7d_vs_mean'], bins=20, alpha=0.7,
    ...
    percentile_values = np.percentile(df['freq_change_7d_vs_mean'], percentiles)
    percentile_fraud = []

    for p in percentiles:
        threshold = np.percentile(df['freq_change_7d_vs_mean'], p)
        high_activity = df[df['freq_change_7d_vs_mean'] >= threshold]
    ...
    for p in key_percentiles:
        ...
        actual_threshold = np.percentile(df['freq_change_7d_vs_mean'], p)
    ...
    for p in [99, 99.5, 99.9]:
        threshold = np.percentile(df['freq_change_7d_vs_mean'], p)
        top_users = df[df['freq_change_7d_vs_mean'] >= threshold]
        if len(top_users) > 0:
...
def nonlinear_analysis(df):
    ...
    sorted_df = df.copy().sort_values('freq_change_7d_vs_mean').reset_index(drop=True)
    ...
    X_poly = valid_data['freq_change_7d_vs_mean'].values
    ...
    ax1.scatter(sorted_df['freq_change_7d_vs_mean'], sorted_df['target'],
                alpha=0.1, color='gray', s=1, label='Исходные данные')
    ax1.plot(valid_data['freq_change_7d_vs_mean'], valid_data['rolling_fraud'] * 100,
...
def kde_analysis(df):
    ...
    logins_0 = df[df['target'] == 0]['freq_change_7d_vs_mean']
    logins_1 = df[df['target'] == 1]['freq_change_7d_vs_mean']
...
def final_report(df, trend_data, r_squared, likelihood_ratio, x_range):
    ...
    correlation = df['freq_change_7d_vs_mean'].corr(df['target'])
    ...
    high_risk_data = df[df['freq_change_7d_vs_mean'] >= high_risk_threshold]
    medium_risk_data = df[(df['freq_change_7d_vs_mean'] >= medium_risk_threshold) &
                          (df['freq_change_7d_vs_mean'] < high_risk_threshold)]
...
def calculate_detailed_stats(df, value_column='freq_change_7d_vs_mean', target_column='target'):
    """
...
def print_detailed_stats(stats_df):
    ...
    print("\n" + "=" * 100)
    print("📊 ДЕТАЛИЗИРОВАННАЯ СТАТИСТИКА ДЛЯ freq_change_7d_vs_mean")
    print("=" * 100)
...
if __name__ == '__main__':
    if num == 0:
        ...
        # stats_df = calculate_detailed_stats(df, value_column='freq_change_7d_vs_mean', target_column='target')
        # formatted_stats = print_detailed_stats(stats_df)

    elif num == 2:
        df = load_data()
        bins = [-float('inf'), 0.0, 1.0, float('inf')]
        labels = ["Базовый риск", "Средний риск", "Высокий риск"]
        result = analyze_manual_bins(
            df=df,
            feature='freq_change_7d_vs_mean',
            target='target',
            bins=bins,
            bin_labels=labels
        )

        visualize_bin_analysis(result, 'freq_change_7d_vs_mean')