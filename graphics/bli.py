...
def compare_with_join():
    ...
    try:
        ...
        result_cursor.execute("""
            CREATE TABLE burstiness_login_interval AS
            SELECT 
                i.cst_dim_id,
                i.transdate,
                i.burstiness_login_interval,
                p.target
            ...
            WHERE i.burstiness_login_interval IS NOT NULL
        """)

        # Статистика
        result_cursor.execute("SELECT COUNT(*) FROM burstiness_login_interval")
        total_count = result_cursor.fetchone()[0]

        result_cursor.execute("SELECT COUNT(*) FROM burstiness_login_interval WHERE target IS NOT NULL")
        matched_count = result_cursor.fetchone()[0]

        result_cursor.execute("SELECT COUNT(*) FROM burstiness_login_interval WHERE target = 1")
        target_1_count = result_cursor.fetchone()[0]

        result_cursor.execute("SELECT COUNT(*) FROM burstiness_login_interval WHERE target = 0")
        ...
        # Выборка результатов
        result_cursor.execute("SELECT * FROM burstiness_login_interval LIMIT 10")
        sample_results = result_cursor.fetchall()

        print("\n🔍 ПЕРВЫЕ 10 РЕЗУЛЬТАТОВ:")
        print("ID | Дата | burstiness_login_interval | Target")
        print("-" * 50)
        ...
        """)
        result_cursor.execute("""
            CREATE TABLE results_db.burstiness_login_interval AS
            SELECT * FROM burstiness_login_interval
        """)
...
def load_data():
    ...
    df = pd.read_sql_query(
        "SELECT burstiness_login_interval, target FROM burstiness_login_interval WHERE target IS NOT NULL",
        conn
    )
    conn.close()
    return df
...
def automatic_binning_analysis(df):
    ...
    X = df['burstiness_login_interval'].values.reshape(-1, 1)
    ...
    for idx, (strategy, name) in enumerate(strategies.items()):
        ...
        bin_stats = df_binned.groupby('bin').agg({
            'burstiness_login_interval': ['min', 'max', 'count'],
            'target': ['sum', 'mean']
        }).round(3)
...
def trend_analysis(df):
    ...
    df['bin'] = pd.cut(df['burstiness_login_interval'], bins=bins, labels=labels, right=False)
    trend_data = df.groupby('bin').agg({
        'target': ['count', 'sum', 'mean'],
        'burstiness_login_interval': 'median'
    }).round(4)
...
def tail_analysis(df):
    ...
    mean_logins = df['burstiness_login_interval'].mean()
    std_logins = df['burstiness_login_interval'].std()
    outlier_threshold = mean_logins + 3 * std_logins

    outliers = df[df['burstiness_login_interval'] > outlier_threshold]
    normal_data = df[df['burstiness_login_interval'] <= outlier_threshold]
    ...
    for threshold in extreme_thresholds:
        extreme_data = df[df['burstiness_login_interval'] >= threshold]
    ...
    ax1.hist(normal_data['burstiness_login_interval'], bins=50, alpha=0.7,
             color='blue', label=f'Нормальные (≤{outlier_threshold:.0f})')
    ax1.hist(outliers['burstiness_login_interval'], bins=20, alpha=0.7,
    ...
    percentile_values = np.percentile(df['burstiness_login_interval'], percentiles)
    percentile_fraud = []

    for p in percentiles:
        threshold = np.percentile(df['burstiness_login_interval'], p)
        high_activity = df[df['burstiness_login_interval'] >= threshold]
    ...
    for p in key_percentiles:
        ...
        actual_threshold = np.percentile(df['burstiness_login_interval'], p)
    ...
    for p in [99, 99.5, 99.9]:
        threshold = np.percentile(df['burstiness_login_interval'], p)
        top_users = df[df['burstiness_login_interval'] >= threshold]
        if len(top_users) > 0:
...
def nonlinear_analysis(df):
    ...
    sorted_df = df.copy().sort_values('burstiness_login_interval').reset_index(drop=True)
    ...
    X_poly = valid_data['burstiness_login_interval'].values
    ...
    ax1.scatter(sorted_df['burstiness_login_interval'], sorted_df['target'],
                alpha=0.1, color='gray', s=1, label='Исходные данные')
    ax1.plot(valid_data['burstiness_login_interval'], valid_data['rolling_fraud'] * 100,
...
def kde_analysis(df):
    ...
    logins_0 = df[df['target'] == 0]['burstiness_login_interval']
    logins_1 = df[df['target'] == 1]['burstiness_login_interval']
...
def final_report(df, trend_data, r_squared, likelihood_ratio, x_range):
    ...
    correlation = df['burstiness_login_interval'].corr(df['target'])
    ...
    high_risk_data = df[df['burstiness_login_interval'] >= high_risk_threshold]
    medium_risk_data = df[(df['burstiness_login_interval'] >= medium_risk_threshold) &
                          (df['burstiness_login_interval'] < high_risk_threshold)]
...
def calculate_detailed_stats(df, value_column='burstiness_login_interval', target_column='target'):
    """
...
def print_detailed_stats(stats_df):
    ...
    print("\n" + "=" * 100)
    print("📊 ДЕТАЛИЗИРОВАННАЯ СТАТИСТИКА ДЛЯ burstiness_login_interval")
    print("=" * 100)
...
if __name__ == '__main__':
    if num == 0:
        ...
        # stats_df = calculate_detailed_stats(df, value_column='burstiness_login_interval', target_column='target')
        # formatted_stats = print_detailed_stats(stats_df)

    elif num == 2:
        df = load_data()
        # Группировка для burstiness_login_interval
        bins = [-1.0, -0.7, 0.4, 0.683]
        labels = ['A. Высокий Риск', 'B. Средний Риск', 'C. Низкий Риск']
        result = analyze_manual_bins(
            df=df,
            feature='burstiness_login_interval',
            target='target',
            bins=bins,
            bin_labels=labels
        )

        visualize_bin_analysis(result, 'burstiness_login_interval')