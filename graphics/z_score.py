...
def compare_with_join():
    ...
    try:
        ...
        result_cursor.execute("""
            CREATE TABLE zscore_avg_login_interval_7d AS
            SELECT 
                i.cst_dim_id,
                i.transdate,
                i.zscore_avg_login_interval_7d,
                p.target
            ...
            WHERE i.zscore_avg_login_interval_7d IS NOT NULL
        """)

        # Статистика
        result_cursor.execute("SELECT COUNT(*) FROM zscore_avg_login_interval_7d")
        total_count = result_cursor.fetchone()[0]

        result_cursor.execute("SELECT COUNT(*) FROM zscore_avg_login_interval_7d WHERE target IS NOT NULL")
        matched_count = result_cursor.fetchone()[0]

        result_cursor.execute("SELECT COUNT(*) FROM zscore_avg_login_interval_7d WHERE target = 1")
        target_1_count = result_cursor.fetchone()[0]

        result_cursor.execute("SELECT COUNT(*) FROM zscore_avg_login_interval_7d WHERE target = 0")
        ...
        # Выборка результатов
        result_cursor.execute("SELECT * FROM zscore_avg_login_interval_7d LIMIT 10")
        sample_results = result_cursor.fetchall()

        print("\n🔍 ПЕРВЫЕ 10 РЕЗУЛЬТАТОВ:")
        print("ID | Дата | zscore_avg_login_interval_7d | Target")
        print("-" * 50)
        ...
        """)
        result_cursor.execute("""
            CREATE TABLE results_db.zscore_avg_login_interval_7d AS
            SELECT * FROM zscore_avg_login_interval_7d
        """)
...
def load_data():
    ...
    df = pd.read_sql_query(
        "SELECT zscore_avg_login_interval_7d, target FROM zscore_avg_login_interval_7d WHERE target IS NOT NULL",
        conn
    )
    conn.close()
    return df
...
def automatic_binning_analysis(df):
    ...
    X = df['zscore_avg_login_interval_7d'].values.reshape(-1, 1)
    ...
    for idx, (strategy, name) in enumerate(strategies.items()):
        ...
        bin_stats = df_binned.groupby('bin').agg({
            'zscore_avg_login_interval_7d': ['min', 'max', 'count'],
            'target': ['sum', 'mean']
        }).round(3)
...
def trend_analysis(df):
    ...
    df['bin'] = pd.cut(df['zscore_avg_login_interval_7d'], bins=bins, labels=labels, right=False)
    trend_data = df.groupby('bin').agg({
        'target': ['count', 'sum', 'mean'],
        'zscore_avg_login_interval_7d': 'median'
    }).round(4)
...
def tail_analysis(df):
    ...
    mean_logins = df['zscore_avg_login_interval_7d'].mean()
    std_logins = df['zscore_avg_login_interval_7d'].std()
    outlier_threshold = mean_logins + 3 * std_logins

    outliers = df[df['zscore_avg_login_interval_7d'] > outlier_threshold]
    normal_data = df[df['zscore_avg_login_interval_7d'] <= outlier_threshold]
    ...
    for threshold in extreme_thresholds:
        extreme_data = df[df['zscore_avg_login_interval_7d'] >= threshold]
    ...
    ax1.hist(normal_data['zscore_avg_login_interval_7d'], bins=50, alpha=0.7,
             color='blue', label=f'Нормальные (≤{outlier_threshold:.0f})')
    ax1.hist(outliers['zscore_avg_login_interval_7d'], bins=20, alpha=0.7,
    ...
    percentile_values = np.percentile(df['zscore_avg_login_interval_7d'], percentiles)
    percentile_fraud = []

    for p in percentiles:
        threshold = np.percentile(df['zscore_avg_login_interval_7d'], p)
        high_activity = df[df['zscore_avg_login_interval_7d'] >= threshold]
    ...
    for p in key_percentiles:
        ...
        actual_threshold = np.percentile(df['zscore_avg_login_interval_7d'], p)
    ...
    for p in [99, 99.5, 99.9]:
        threshold = np.percentile(df['zscore_avg_login_interval_7d'], p)
        top_users = df[df['zscore_avg_login_interval_7d'] >= threshold]
        if len(top_users) > 0:
...
def nonlinear_analysis(df):
    ...
    sorted_df = df.copy().sort_values('zscore_avg_login_interval_7d').reset_index(drop=True)
    ...
    X_poly = valid_data['zscore_avg_login_interval_7d'].values
    ...
    ax1.scatter(sorted_df['zscore_avg_login_interval_7d'], sorted_df['target'],
                alpha=0.1, color='gray', s=1, label='Исходные данные')
    ax1.plot(valid_data['zscore_avg_login_interval_7d'], valid_data['rolling_fraud'] * 100,
...
def kde_analysis(df):
    ...
    logins_0 = df[df['target'] == 0]['zscore_avg_login_interval_7d']
    logins_1 = df[df['target'] == 1]['zscore_avg_login_interval_7d']
...
def final_report(df, trend_data, r_squared, likelihood_ratio, x_range):
    ...
    correlation = df['zscore_avg_login_interval_7d'].corr(df['target'])
    ...
    high_risk_data = df[df['zscore_avg_login_interval_7d'] >= high_risk_threshold]
    medium_risk_data = df[(df['zscore_avg_login_interval_7d'] >= medium_risk_threshold) &
                          (df['zscore_avg_login_interval_7d'] < high_risk_threshold)]
...
def calculate_detailed_stats(df, value_column='zscore_avg_login_interval_7d', target_column='target'):
    """
...
def print_detailed_stats(stats_df):
    ...
    print("\n" + "=" * 100)
    print("📊 ДЕТАЛИЗИРОВАННАЯ СТАТИСТИКА ДЛЯ zscore_avg_login_interval_7d")
    print("=" * 100)
...
if __name__ == '__main__':
    if num == 0:
        ...
        # stats_df = calculate_detailed_stats(df, value_column='zscore_avg_login_interval_7d', target_column='target')
        # formatted_stats = print_detailed_stats(stats_df)

    elif num == 2:
        df = load_data()
        bins = [-2.0, -0.2, 0.2, 5.0]
        labels = ['C. Базовый Риск', 'A. Максимальный Риск', 'B. Повышенный Риск']
        result = analyze_manual_bins(
            df=df,
            feature='zscore_avg_login_interval_7d',
            target='target',
            bins=bins,
            bin_labels=labels
        )

        visualize_bin_analysis(result, 'zscore_avg_login_interval_7d')